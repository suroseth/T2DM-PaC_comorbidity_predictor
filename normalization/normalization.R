...
library(sva)
input_data =  "result_preprocessing_two_sample.csv"
if(length(input_data) >= 1)
{
  ...
  data_platform3 = input_data[xx3,]
  
  identical(row.names(data_platform1),row.names(data_platform2))
  combined_data = cbind(data_platform1,data_platform2,data_platform3)
  ...
  batch <- factor(c(rep("Platform1", ncol(data_platform1)),
                    ...
                    rep("Platform3", ncol(data_platform3))))
  normalized_data_combat <- ComBat(combined_data, 
                                   batch = batch, 
                                   mod = NULL, 
                                   par.prior = TRUE, 
                                   prior.plots = FALSE)
  normalized_data_combat = normalized_data_combat[,colnames(normalized_data_combat) %in% colnames(input_data)]  
}else
  {
  eset_gse15932 = read.csv("result_preprocessing_gse15932_3.csv",row.names = 1)
  eset_gse74629 = read.csv("result_preprocessing_GSE74629_3.csv",row.names = 1)
...
data_67_genes_2 = cbind(row.names(data_67_genes),data_67_genes)
colnames(data_67_genes_2)[1] = "genes"
